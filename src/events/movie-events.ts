...
const consumer = kafka.consumer({ groupId: `movie-group-${Date.now()}` })
...
export const consumeMovieEvents = async () => {
  try {
    ...
    console.error(`\n Kafka broker unavailable, skipping event consumption
            \n Kafka is purely optional, but is is there to test events (offline) with pact
            \n If you want to see Kafka working, start the provider/producer server,
            \n start docker, and run 'npm run kafka:start' \n`)
